...
class CompatPostOtherSelectionInstrumentedTest {
    ...
    private fun showScreen(delayDraft: Boolean = false) {
        ...
        runBlocking {
            ...
            store.openTab(tab)
            store.saveDraft(CompatReplyDraft(tabKey = tab.key, comment = "prefix", updatedAtEpochMillis = 1))
        }
    ...
    @Test fun delayedDraftRestorationOverwritesNewInput() {
        showScreen(delayDraft = true)
        typeSuffix()
        draftReady.complete(Unit)
        rule.waitUntil(5_000) { currentText() == "prefix" }
    }
