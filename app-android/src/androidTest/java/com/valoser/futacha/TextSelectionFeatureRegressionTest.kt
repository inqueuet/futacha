...
class TextSelectionFeatureRegressionTest {
    ...
    @Test fun smartSelectionExpandsRange() {
    ...
    @Test fun smartMenuActionRemainsAvailable() {
        ...
        // Verify the menu independently of expansion: a combined expansion/menu timing
        // diagnostic intermittently loses its action even in unpatched Foundation 1.12.0.
        // Keep that comparison and its failure logs in quality/repro/text-selection-fix/.
        val classification = TextClassification.Builder().addAction(action).build()
        injectedClassification = classification
        injectedResponse = { TextSelection.Builder(it.startIndex, it.endIndex)
