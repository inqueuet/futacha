-- Futacha compatibility database schema export, version 6.
...
CREATE TABLE compat_thread_snapshot(tab_key TEXT PRIMARY KEY NOT NULL, revision INTEGER NOT NULL, fetched_at INTEGER NOT NULL, board_title TEXT, expires_label TEXT, deleted_notice TEXT, FOREIGN KEY(tab_key) REFERENCES compat_tab(tab_key) ON DELETE CASCADE);
CREATE TABLE compat_post(tab_key TEXT NOT NULL, revision INTEGER NOT NULL, position INTEGER NOT NULL, post_json TEXT NOT NULL, PRIMARY KEY(tab_key, revision, position), FOREIGN KEY(tab_key) REFERENCES compat_tab(tab_key) ON DELETE CASCADE);
CREATE TABLE compat_catalog_preference(board_key TEXT PRIMARY KEY NOT NULL, sort_mode TEXT NOT NULL, layout_mode TEXT NOT NULL, reply_priority_enabled INTEGER NOT NULL, few_replies_delay INTEGER NOT NULL, FOREIGN KEY(board_key) REFERENCES compat_board(board_key) ON DELETE CASCADE);
