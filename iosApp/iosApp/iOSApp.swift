...
private final class FutachaNetworkPathMonitor {
    static let shared = FutachaNetworkPathMonitor()

    private let monitor = NWPathMonitor()
    private let queue = DispatchQueue(label: "com.valoser.futacha.network-path")
...
private enum FutachaFirebaseTelemetryBridge {
    private static var traces: [String: Trace] = [:]
    private static let lock = NSLock()

    static func install() {
        let bridge = IosFirebaseTelemetryBridge.shared

        bridge.setAnalyticsCollectionEnabledHandler = { enabled in
            Analytics.setAnalyticsCollectionEnabled(enabled.boolValue)
        }

        bridge.logEventHandler = { name, keys, values in
            var parameters: [String: String] = [:]
            for (key, value) in zip(keys, values) {
                // Firebase rejects parameter names beginning with `firebase_`.
                // The shared tracker intentionally uses platform-neutral keys,
                // so translate only the two screen fields at the iOS boundary.
                // This preserves Android's existing payload while making iOS
                // screen events visible in Firebase instead of silently dropped.
                let firebaseKey: String
                switch key {
                case "firebase_screen":
                    firebaseKey = "screen_name"
                case "firebase_screen_class":
                    firebaseKey = "screen_class"
                default:
                    firebaseKey = key
                }
                parameters[firebaseKey] = value
            }
            Analytics.logEvent(name, parameters: parameters)
        }

        bridge.setPerformanceCollectionEnabledHandler = { enabled in
            Performance.sharedInstance().isDataCollectionEnabled = enabled.boolValue
        }

        bridge.setCrashlyticsCollectionEnabledHandler = { enabled in
            Crashlytics.crashlytics().setCrashlyticsCollectionEnabled(enabled.boolValue)
        }

        bridge.setCrashlyticsCustomKeyHandler = { name, value in
            Crashlytics.crashlytics().setCustomValue(value, forKey: name)
        }

        bridge.logCrashlyticsMessageHandler = { message in
        ...
        bridge.putTraceAttributeHandler = { traceId, name, value in
            guard let trace = trace(for: traceId) else {
                return
            }
            trace.setValue(value, forAttribute: name)
        }

        bridge.putTraceMetricHandler = { traceId, name, value in
            guard let trace = trace(for: traceId) else {
                return
            }
            trace.incrementMetric(name, by: value.int64Value)
        }

        bridge.stopTraceHandler = { traceId in
            guard let trace = removeTrace(for: traceId) else {
                return
            }
            trace.stop()
        }
    ...
    private static func trace(for traceId: String) -> Trace? {
        lock.lock()
        defer { lock.unlock() }
        return traces[traceId]
    }

    private static func removeTrace(for traceId: String) -> Trace? {
        lock.lock()
        defer { lock.unlock() }
        return traces.removeValue(forKey: traceId)
    }
}
    ...
    var body: some Scene {
        WindowGroup {
            ContentView()
                ...
                .onOpenURL { url in
                    if SavedHtmlDocumentStore.shared.openIfSupported(url) {
                        return
                    }
                    let raw = url.absoluteString
                    if MainViewControllerKt.submitIosThreadDeepLink(raw: raw) {
                        return
                    }
                    if !submitFutachaAiDeepLink(raw) {
                        NSLog("Failed to enqueue Futacha deep link")
                    }
                }
...
private final class FutachaAiDeepLinkSubmitter {
    ...
    private let maxDeepLinkCharacters = 64 * 1024
    ...
    func submit(_ raw: String) -> Bool {
        ...
        guard raw.count <= maxDeepLinkCharacters else {
            return false
        }
