...
fun SavedThreadMetadata.toThreadPage(
    ...
    val resolvedLocalBase by lazy {
        when (val location = baseSaveLocation) {
            null -> fileSystem.resolveAbsolutePath("$baseDirectory/$storageFolder").trimEnd('/')
            is SaveLocation.Path -> {
                val resolvedBase = fileSystem.resolveAbsolutePath(location.path).trimEnd('/')
                "$resolvedBase/$storageFolder"
            }
    ...
    fun resolveLocalPath(relativePath: String?): String? {
        ...
        return when (val location = baseSaveLocation) {
            null,
            is SaveLocation.Path -> "${requireNotNull(resolvedLocalBase)}/$normalizedRelativePath"
