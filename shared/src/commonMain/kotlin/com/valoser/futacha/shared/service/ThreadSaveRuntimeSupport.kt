...
internal fun buildThreadSaveMetadataPayloadWithStableSize(
    ...
    require(baseTotalSize >= 0L) { "baseTotalSize must be non-negative" }
    ...
    repeat(8) {
        ...
        estimatedSize = size
        payload = encodeMetadata(metadata.copy(totalSize = checkedThreadSaveSizeAdd(baseTotalSize, size)))
    }
...
internal fun resolveThreadSaveFileName(
    ...
    val safeExtension = extension
        ...
        .take(12)
        .ifBlank { "bin" }
    val fileName = candidate ?: "${safePostId}_${timestampMillis}.$safeExtension"
...
internal fun checkedThreadSaveSizeAdd(left: Long, right: Long): Long {
    require(left >= 0L && right >= 0L) { "Thread save sizes must be non-negative" }
    if (right > Long.MAX_VALUE - left) {
        throw IllegalStateException("Thread save size overflow")
    }
    return left + right
}
