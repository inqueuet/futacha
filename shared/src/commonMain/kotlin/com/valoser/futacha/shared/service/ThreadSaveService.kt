...
import com.valoser.futacha.shared.repository.requireSavedThreadMetadataWithinLimits
...
class ThreadSaveService(
    ...
    companion object {
        // Futaba videos regularly exceed the old 64 MiB guard. Downloads are
        // streamed to disk, so use a per-item ceiling that protects storage
        // without rejecting ordinary long MP4/WebM attachments.
        private const val MAX_FILE_SIZE_BYTES = 512L * 1024L * 1024L
        private const val MAX_THREAD_HTML_BYTES = 20L * 1024L * 1024L
        private const val MAX_EXISTING_METADATA_BYTES = 32L * 1024L * 1024L
        private const val MAX_TOTAL_SIZE_BYTES = 8L * 1024L * 1024L * 1024L
        // Large Futaba threads can contain thousands of images/videos. Keep
        // the internal budget aligned with the platform's 3-hour protection;
        // otherwise the service would still abort at the former 15-minute
        // ceiling even though the UI and foreground service remain alive.
        const val DEFAULT_MAX_SAVE_DURATION_MS = 3 * 60 * 60 * 1000L
        ...
        // FIX: 最大メディア数を制限
        const val DEFAULT_MAX_MEDIA_ITEMS = 3_000
        ...
        private const val READ_IDLE_TIMEOUT_MILLIS = THREAD_SAVE_READ_IDLE_TIMEOUT_MILLIS
    ...
    private suspend fun readThreadSaveMetadataPayload(
        ...
        return runSuspendCatchingPreservingCancellation {
            ...
            withContext(AppDispatchers.parsing) {
                ExistingThreadSaveMetadata(
                    metadata = requireSavedThreadMetadataWithinLimits(
                        json.decodeFromString<SavedThreadMetadata>(payload)
                    ),
                    payload = payload
                )
            }
    ...
    private suspend fun readThreadSaveText(
        ...
        val sizeBeforeRead = if (target.saveLocation != null) {
        ...
        require(sizeBeforeRead in 0L..MAX_EXISTING_METADATA_BYTES) {
            "Existing thread metadata is too large"
        }
        val payload = if (target.saveLocation != null) {
        ...
        require(payload.encodeToByteArray().size.toLong() <= MAX_EXISTING_METADATA_BYTES) {
            "Existing thread metadata grew beyond its permitted size while reading"
        }
        return payload
    }
    ...
    private suspend fun measureExistingThreadSaveMedia(
        ...
        return runSuspendCatchingPreservingCancellation {
            ...
            if (!exists) return@runSuspendCatchingPreservingCancellation null
    ...
    /** Clears a completed run before a new progress dialog is presented. */
