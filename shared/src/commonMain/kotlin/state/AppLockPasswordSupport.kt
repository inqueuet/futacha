...
internal const val APP_LOCK_PASSWORD_MAX_LENGTH = 256
private const val APP_LOCK_PASSWORD_HASH_MAX_LENGTH = 128
private const val APP_LOCK_SALT_MAX_LENGTH = 64

internal const val APP_LOCK_PASSWORD_MIN_LENGTH = 4

internal fun sanitizeAppLockPasswordHash(raw: String?): String? {
    if (raw == null || raw.length > APP_LOCK_PASSWORD_HASH_MAX_LENGTH) return null
    val trimmed = raw.trim()
    if (trimmed.isBlank()) return null
    return trimmed.takeIf {
        it.startsWith("$APP_LOCK_PASSWORD_HASH_PREFIX:") &&
...
internal fun buildAppLockPasswordHash(
    ...
    require(isValidAppLockPassword(password)) { "App lock password length is invalid" }
    require(salt.isNotBlank() && salt.length <= APP_LOCK_SALT_MAX_LENGTH) { "App lock salt is invalid" }
...
internal fun verifyAppLockPassword(
    ...
    if (!isValidAppLockPassword(password)) return false
    ...
    if (salt.isBlank() || salt.length > APP_LOCK_SALT_MAX_LENGTH || expectedDigest.length != 64) return false
...
internal fun isValidAppLockPassword(password: String): Boolean {
    return password.length in APP_LOCK_PASSWORD_MIN_LENGTH..APP_LOCK_PASSWORD_MAX_LENGTH
}
