...
internal class AppStatePreferenceSnapshotCoordinator(
    ...
    private var cachedBoardWatchWordMap: Map<String, List<String>>? = null
    ...
    suspend fun mutateBoardWatchWordMap(
        transform: (Map<String, List<String>>) -> Map<String, List<String>>
    ) {
        val loadedSnapshot = readBoardWatchWordMapSnapshot()
        mutateAppStateCachedSnapshot(
            mutex = boardWatchWordsMutex,
            loadedSnapshot = loadedSnapshot,
            currentCachedValue = { cachedBoardWatchWordMap },
            setCachedValue = { cachedBoardWatchWordMap = it },
            transform = transform,
            persistUpdatedValue = ::persistBoardWatchWordMap,
            onWriteFailure = { error -> throw error },
            rethrowIfCancellation = rethrowIfCancellation
        )
    }
    ...
    private suspend fun readBoardWatchWordMapSnapshot(): Map<String, List<String>> {
        return readAppStateCachedSnapshot(
            mutex = boardWatchWordsMutex,
            currentCachedValue = { cachedBoardWatchWordMap },
            readStorageSnapshot = {
                val raw = storage.boardWatchWordsJson.first()
                withContext(AppDispatchers.parsing) {
                    decodeStringListMapValue(raw, json, stringListMapSerializer)
                }
            },
            setCachedValue = { cachedBoardWatchWordMap = it },
            onReadFailure = { error ->
                Logger.e(tag, "Failed to read board watch word map", error)
                emptyMap()
            },
            rethrowIfCancellation = rethrowIfCancellation
        )
    ...
    private suspend fun persistBoardWatchWordMap(map: Map<String, List<String>>) {
        val encoded = json.encodeToString(stringListMapSerializer, map)
        storage.updateBoardWatchWordsJson(encoded)
    }
