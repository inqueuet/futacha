...
internal class AppStatePreferenceOperations(
    ...
    private val stringListMapSerializer: KSerializer<Map<String, List<String>>>,
    ...
    private val mutateBoardWatchWordMap: suspend (
        transform: (Map<String, List<String>>) -> Map<String, List<String>>
    ) -> Unit
) {
    ...
    suspend fun setUpdateCheckEnabled(enabled: Boolean) {
        setSimple(enabled, storage::updateUpdateCheckEnabled, "update check state")
    }

    suspend fun setWatchAlertEnabled(enabled: Boolean) {
        setSimple(enabled, storage::updateWatchAlertEnabled, "watch alert state")
    }
    ...
    suspend fun setTelemetryCollectionEnabled(enabled: Boolean) {
        setSimple(enabled, storage::updateTelemetryCollectionEnabled, "telemetry collection state")
    }
    ...
    suspend fun setThreadGalleryThumbnailMode(mode: ThreadGalleryThumbnailMode) {
        setSimple(mode.name, storage::updateThreadGalleryThumbnailMode, "thread gallery thumbnail mode")
    }
    ...
    suspend fun setCompactThreadHeaderEnabled(enabled: Boolean) {
        setSimple(enabled, storage::updateCompactThreadHeaderEnabled, "compact thread header state")
    }
    ...
    suspend fun setCatalogFetchRows(rows: Int) {
        val clamped = normalizeCatalogFetchRows(rows)
        setSimple(clamped.toString(), storage::updateCatalogFetchRows, "catalog fetch rows")
    }
    ...
    suspend fun setNgHeaders(headers: List<String>) {
        setList(normalizeStoredStringList(headers), stringListSerializer, storage::updateNgHeadersJson, "NG headers")
    }

    suspend fun setNgWords(words: List<String>) {
        setList(normalizeStoredStringList(words), stringListSerializer, storage::updateNgWordsJson, "NG words")
    }

    suspend fun setCatalogNgWords(words: List<String>) {
        setList(normalizeStoredStringList(words), stringListSerializer, storage::updateCatalogNgWordsJson, "catalog NG words")
    }

    suspend fun setWatchWords(words: List<String>) {
        setList(normalizeStoredStringList(words), stringListSerializer, storage::updateWatchWordsJson, "watch words")
    }
    ...
    suspend fun setBoardWatchWords(boardId: String, words: List<String>) {
        ...
        if (normalizedBoardId.isBlank()) {
            Logger.w(tag, "Ignoring board watch words update with blank boardId")
            return
        }
        val sanitizedWords = normalizeStoredStringList(words)
        try {
            mutateBoardWatchWordMap { current ->
                current + (normalizedBoardId to sanitizedWords)
            }
        } catch (error: Exception) {
            rethrowIfCancellation(error)
            Logger.e(tag, "Failed to save board watch words for $normalizedBoardId (${sanitizedWords.size})", error)
        }
    }
    ...
    suspend fun clearBoardWatchWordsOverride(boardId: String) {
        val normalizedBoardId = boardId.trim()
        if (normalizedBoardId.isBlank()) {
            Logger.w(tag, "Ignoring board watch words override clear with blank boardId")
            return
        }
        try {
            mutateBoardWatchWordMap { current -> current - normalizedBoardId }
        } catch (error: Exception) {
            rethrowIfCancellation(error)
            Logger.e(tag, "Failed to clear board watch words override for $normalizedBoardId", error)
        }
    }
