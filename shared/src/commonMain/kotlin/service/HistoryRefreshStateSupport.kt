...
internal class HistoryRefreshUpdateBuffer(
    ...
    private val commitHistoryEntries: suspend (Collection<ThreadHistoryEntry>) -> Boolean,
    ...
    suspend fun flush(force: Boolean, failureStage: String): Boolean {
        ...
        val effectiveMaxFlushRetries = maxFlushRetries.coerceIn(1, 10)
        repeat(effectiveMaxFlushRetries) { attempt ->
            try {
                if (!commitHistoryEntries(flushSnapshot.values)) {
                    throw HistoryRefreshCommitRejectedException()
                }
                updatesMutex.withLock {
                ...
                lastError = error
                val isLastAttempt = attempt >= effectiveMaxFlushRetries - 1
                if (!isLastAttempt) {
                    Logger.w(
                        tag,
                        "Retrying history flush (${attempt + 1}/$effectiveMaxFlushRetries): ${error.message}"
                    )
                    val retryDelay = calculateHistoryFlushRetryDelay(
                        initialDelayMillis = flushRetryDelayMillis,
                        maxDelayMillis = flushRetryMaxDelayMillis,
                        attempt = attempt
                    )
                    delay(retryDelay)
                    yield()
        ...
        Logger.e(
            tag,
            "Failed to flush ${flushSnapshot.size} history updates after $effectiveMaxFlushRetries attempts",
            failure
        )
        recordError(
            "history-flush",
...
internal fun calculateHistoryFlushRetryDelay(
    initialDelayMillis: Long,
    maxDelayMillis: Long,
    attempt: Int
): Long {
    val safeMax = maxDelayMillis.coerceAtLeast(0L)
    var delayMillis = initialDelayMillis.coerceIn(0L, safeMax)
    repeat(attempt.coerceIn(0, 62)) {
        delayMillis = if (delayMillis >= safeMax - delayMillis) safeMax else delayMillis * 2L
    }
    return delayMillis
}

class HistoryRefreshCommitRejectedException :
    IllegalStateException("History refresh commit rejected because its experience generation is stale")
