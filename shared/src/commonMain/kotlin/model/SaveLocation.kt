...
internal const val MAX_SAVE_LOCATION_BOOKMARK_BASE64_CHARS = 1024 * 1024
...
sealed class SaveLocation {
    ...
    companion object {
        ...
        private fun looksLikeLegacyBookmark(raw: String): Boolean {
            if (raw.isBlank() || raw.length > MAX_SAVE_LOCATION_BOOKMARK_BASE64_CHARS) return false
        ...
        private fun decodeBase64Lenient(raw: String): ByteArray? {
            if (raw.length > MAX_SAVE_LOCATION_BOOKMARK_BASE64_CHARS) return null
            val normalized = raw
                .trim()
                .replace("\n", "")
                .replace("\r", "")
