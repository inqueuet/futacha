...
internal object ThreadHtmlParserCore {
    ...
    private val tableRegex = Regex(
        pattern = "<table\\b[^>]{0,500}\\bborder\\s*=\\s*['\"]?0['\"]?[^>]{0,500}>",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    ...
    // FTBucket stores captured media below relative img/ and thumb/ paths,
    // unlike a live Futaba page which normally exposes /src/ URLs.
    private val anyMediaLinkRegex = Regex(
        pattern = "<a[^>]{1,500}href=['\"]([^'\"]+\\.(?:$FUTABA_COMPAT_MEDIA_EXTENSION_PATTERN)(?:[?#][^'\"]*)?)['\"][^>]{0,300}>",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    // FIX: ReDoS対策 - [^>]+に長さ制限を追加
    ...
    private val anyMediaImgRegex = Regex(
        pattern = "<img[^>]{1,500}src=['\"]([^'\"]+\\.(?:$FUTABA_COMPAT_MEDIA_EXTENSION_PATTERN)(?:[?#][^'\"]*)?)['\"][^>]{0,300}>",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    private val mediaImgTagRegex = Regex(
        pattern = "<img\\b[^>]{0,1500}>",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    private val widthAttrRegex = Regex(
        pattern = "\\bwidth\\s*=\\s*['\"]?(\\d+)['\"]?",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    ...
    private val mediaFilenameRegex = Regex(
        pattern = "([A-Za-z0-9._-]+\\.(?:$FUTABA_COMPAT_MEDIA_EXTENSION_PATTERN))",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    private val uploaderMediaFilenameRegex = Regex(
        pattern = "(?<![A-Za-z0-9._/-])((?:fu\\d+|f\\d+)\\.(?:$FUTABA_COMPAT_MEDIA_EXTENSION_PATTERN))(?![A-Za-z0-9._-])",
        options = setOf(RegexOption.IGNORE_CASE)
    )
    ...
    private val opContainerStartRegex = Regex(
        "<(?:(?:div)|(?:span))\\s+class=['\"]thre['\"][^>]*>",
    ...
    private suspend fun parseThreadWithinBudget(html: String, fallbackBaseUrl: String?): ThreadPage {
        return try {
            ...
            val baseUrl = resolveThreadMediaBaseUrl(canonical, fallbackBaseUrl)
            ...
            val firstReplyIndex = tableRegex.find(normalized)?.range?.first ?: -1
    ...
    fun extractOpImageUrl(html: String, baseUrl: String? = null): String? {
        ...
        val resolvedBaseUrl = resolveThreadMediaBaseUrl(canonical, baseUrl)
    ...
    private fun isVideoUrl(url: String): Boolean {
        return mediaFileExtension(url) in videoExtensions
    }
    ...
    private fun extractOpBlock(html: String, firstReplyIndex: Int): String? {
        // Futaba's live pages traditionally use <div class="thre">, while
        // FTBucket's archived captures use <span class="thre">.  Both are
        // the same OP container; use the first reply table as the boundary so
        // nested spans in the archive cannot terminate the block too early.
        val start = opContainerStartRegex.find(html)?.range?.first ?: -1
    ...
    private suspend fun parsePostBlock(
        ...
        val mail = mailRegex.find(block)?.groupValues?.getOrNull(1)
        ...
        val imageUrl = (
            srcLinkRegex.find(attachmentBlock)?.groupValues?.getOrNull(1)
                ?: anyMediaLinkRegex.find(attachmentBlock)?.groupValues?.getOrNull(1)
            )?.let { resolveUrl(it, baseUrl) }
            ?: uploaderMediaUrl
        val thumbnailUrl = (
            thumbImgRegex.find(attachmentBlock)?.groupValues?.getOrNull(1)
                ?: anyMediaImgRegex.find(attachmentBlock)?.groupValues?.getOrNull(1)
            )?.let { resolveUrl(it, baseUrl) }
        val thumbnailTag = mediaImgTagRegex.find(attachmentBlock)?.value
        val thumbnailWidth = thumbnailTag
            ?.let { widthAttrRegex.find(it)?.groupValues?.getOrNull(1)?.toIntOrNull() }
            ?.takeIf { it > 0 }
        ...
        val saidane = saidaneRegex.find(block)
            ...
            ?.trim()
            ?.ifBlank { null }
        // Deleted posts are identified by Futaba's structural marker.  A
    ...
    private fun withoutMessageBlock(block: String): String {
        val start = blockquoteRegex.find(block) ?: return block
        val end = blockquoteEndRegex.find(block, start.range.last + 1) ?: return block
        return block.removeRange(start.range.first, end.range.last + 1)
    }
    ...
    private fun containsIsolationNotice(messageHtml: String): Boolean {
        ...
        val firstLine = decodeHtmlEntities(stripTags(messageHtml))
            ...
            ?: return false
        // The server notice is the leading line. A quote starts with '>' and
        // must remain a normal reply.
        return firstLine == ISOLATION_NOTICE_TEXT
    }

    private fun resolveUploaderMediaUrl(messageHtml: String): String? {
        if (messageHtml.isBlank()) return null
        // A quoted reply can contain the previous post's `fu....` filename.
        // Treating that quote as this post's attachment duplicates the media
        // entry and shifts every later gallery/viewer index.  Only an
        // unquoted line is an attachment candidate.
    ...
    private fun resolveThreadMediaBaseUrl(canonicalUrl: String?, fallbackBaseUrl: String?): String {
        val fallback = fallbackBaseUrl?.trim()?.takeIf { it.isNotBlank() }
        if (fallback != null && isInqueuetArchiveUrl(fallback)) {
            return fallback
        }
        return canonicalUrl?.let(::extractBaseUrl)
            ?: fallback
            ?: DEFAULT_BASE_URL
    }

    private fun isInqueuetArchiveUrl(url: String): Boolean {
        val base = extractBaseUrl(url) ?: return false
        val host = base.substringAfter("://", "").substringBefore('/').substringBefore(':').lowercase().trim('.')
        return host == "inqueuet.com" || host.endsWith(".inqueuet.com")
    }
