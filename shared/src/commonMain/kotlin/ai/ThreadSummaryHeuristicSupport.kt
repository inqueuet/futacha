...
private const val SUMMARY_MAX_POST_SOURCE_CHARS = 12_000
...
fun buildThreadSummarySourceText(input: ThreadSummaryInput): String {
    val lines = input.posts
        ...
        .filter { it.length >= 4 }
        .distinctBy { it.lowercase() }
    return buildString(SUMMARY_MAX_SOURCE_CHARS) {
        val iterator = lines.iterator()
        while (iterator.hasNext() && length < SUMMARY_MAX_SOURCE_CHARS) {
            val line = iterator.next()
            if (isNotEmpty()) append('\n')
            append(line.take(SUMMARY_MAX_SOURCE_CHARS - length))
        }
    }.trim()
}
...
private fun Post.toSummaryBodyLines(): List<String> {
    return stripHtmlTagsLinear(
        replaceHtmlBreakTags(
            messageHtml.take(SUMMARY_MAX_POST_SOURCE_CHARS),
