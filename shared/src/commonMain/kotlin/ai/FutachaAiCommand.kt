    ...
    return parameters.entries
        ...
        .mapNotNull { (key, value) ->
            val sanitizedKey = key
                .take(FUTACHA_AI_COMMAND_MAX_PARAMETER_KEY_CHARS + 1)
                .filterNot { it == '\u0000' }
                .trim()
                .take(FUTACHA_AI_COMMAND_MAX_PARAMETER_KEY_CHARS)
            val sanitizedValue = value
                .take(FUTACHA_AI_COMMAND_MAX_PARAMETER_VALUE_CHARS + 1)
                .filterNot { it == '\u0000' }
                .trim()
...
private fun String.normalizedAiIdentifier(): String {
    return take(FUTACHA_AI_COMMAND_MAX_PARAMETER_KEY_CHARS * 4)
        .trim()
        .lowercase()
