...
private fun parseSemVer(raw: String): SemVer? {
    ...
    val versionWithoutBuildMetadata = trimmed.substringBefore("+")
    val mainAndPre = versionWithoutBuildMetadata.split("-", limit = 2)
    val mainPart = mainAndPre[0].trim()
    val mainSegments = mainPart.split(".")
    if (mainSegments.size !in 1..3 || mainSegments.any { it.isEmpty() || it.any { ch -> !ch.isDigit() } }) {
        return null
    }
    ...
    val preRelease = if (mainAndPre.size > 1) {
        val rawPreRelease = mainAndPre[1]
        val tokens = rawPreRelease.split(".")
        if (tokens.isEmpty() || tokens.any { it.isBlank() }) return null
        tokens.map { token ->
            if (token.any { !it.isLetterOrDigit() && it != '-' }) return null
...
private fun compareSemVer(left: SemVer, right: SemVer): Int {
    ...
    for (i in 0 until max) {
        ...
        if (l.isNumeric && r.isNumeric) {
            val numericComparison = compareNumericSemVerIdentifier(l.value, r.value)
...
private fun compareNumericSemVerIdentifier(left: String, right: String): Int {
