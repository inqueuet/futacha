...
internal fun buildBoardManagementBulkAddSuccessMessage(count: Int): String = "${count}件の板を追加しました"
...
internal data class BoardManagementDialogCallbacks(
    ...
    val onAddBoardsSubmitted: (List<Pair<String, String>>) -> Unit,
...
internal fun buildBoardManagementDialogCallbacks(
    ...
    return BoardManagementDialogCallbacks(
        ...
        onAddBoardsSubmitted = { boards ->
            boards.forEach { (name, url) -> onAddBoard(name, url) }
            setIsAddDialogVisible(false)
            coroutineScope.launch {
                showSnackbar(buildBoardManagementBulkAddSuccessMessage(boards.size))
            }
        },
