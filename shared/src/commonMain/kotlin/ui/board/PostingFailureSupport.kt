...
private const val POSTING_FAILURE_UI_DETAIL_MAX_CHARS = 500
...
internal fun buildPostingAwareFailureMessage(
    ...
    val detail = error.message
        ?.take(POSTING_FAILURE_UI_DETAIL_MAX_CHARS)
        ?.trim()
        .takeUnless { it.isNullOrEmpty() }
        ?: fallbackDetail.orEmpty().take(POSTING_FAILURE_UI_DETAIL_MAX_CHARS).trim()
    if (detail.isEmpty()) {
