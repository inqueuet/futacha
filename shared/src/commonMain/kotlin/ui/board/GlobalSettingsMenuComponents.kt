...
internal fun GlobalSettingsCatalogMenuSection(
    ...
    SettingsSection(
        ...
        Row(
            ...
            TextButton(onClick = {
                recordMenuConfigurationControl("カタログメニューを初期化")
                catalogMenuCallbacks.resetEntries()
            }) {
                Text("リセット")
            }
        }
        ...
        catalogMenuState.allEntries.forEach { item ->
            ...
            Column(
                ...
                FlowRow(
                    ...
                    IconButton(
                        onClick = {
                            recordMenuConfigurationControl("カタログの${meta.label}を左へ移動")
                            catalogMenuCallbacks.moveEntry(item.id, -1)
                        },
                        enabled = canMoveLeft
                    ) {
                    ...
                    IconButton(
                        onClick = {
                            recordMenuConfigurationControl("カタログの${meta.label}を右へ移動")
                            catalogMenuCallbacks.moveEntry(item.id, 1)
                        },
                    ...
                    AssistChip(
                        onClick = {
                            recordMenuConfigurationControl("カタログの${meta.label}をバーに表示")
                            catalogMenuCallbacks.setPlacement(item.id, CatalogNavEntryPlacement.BAR)
                        },
                        label = { Text("バー") },
                    ...
                    AssistChip(
                        onClick = {
                            recordMenuConfigurationControl("カタログの${meta.label}を非表示")
                            catalogMenuCallbacks.setPlacement(item.id, CatalogNavEntryPlacement.HIDDEN)
                        },
...
internal fun GlobalSettingsThreadMenuSection(
    ...
    SettingsSection(
        ...
        Row(
            ...
            TextButton(onClick = {
                recordMenuConfigurationControl("スレッドメニューを初期化")
                threadMenuCallbacks.resetEntries()
            }) {
                Text("リセット")
            }
        }
        ...
        threadMenuState.allEntries.forEach { item ->
            ...
            Column(
                ...
                FlowRow(
                    ...
                    IconButton(
                        onClick = {
                            recordMenuConfigurationControl("スレッドの${meta.label}を左へ移動")
                            when (placement) {
                    ...
                    IconButton(
                        onClick = {
                            recordMenuConfigurationControl("スレッドの${meta.label}を右へ移動")
                            when (placement) {
                                ThreadMenuEntryPlacement.BAR -> threadMenuCallbacks.moveWithinPlacement(item.id, 1, ThreadMenuEntryPlacement.BAR)
                                ThreadMenuEntryPlacement.SHEET -> threadMenuCallbacks.moveWithinPlacement(item.id, 1, ThreadMenuEntryPlacement.SHEET)
                                ThreadMenuEntryPlacement.HIDDEN -> Unit
                    ...
                    AssistChip(
                        onClick = {
                            recordMenuConfigurationControl("スレッドの${meta.label}をバーに表示")
                            threadMenuCallbacks.setPlacement(item.id, ThreadMenuEntryPlacement.BAR)
                        },
                        label = { Text("バー") },
                    ...
                    AssistChip(
                        onClick = {
                            recordMenuConfigurationControl("スレッドの${meta.label}を設定に表示")
                            threadMenuCallbacks.setPlacement(item.id, ThreadMenuEntryPlacement.SHEET)
                        },
                        label = { Text("設定") },
                    ...
                    AssistChip(
                        onClick = {
                            recordMenuConfigurationControl("スレッドの${meta.label}を非表示")
                            threadMenuCallbacks.setPlacement(item.id, ThreadMenuEntryPlacement.HIDDEN)
                        },
                        label = { Text("非表示") },
...
private fun recordMenuConfigurationControl(label: String) {
    AnalyticsTracker.uiControl("menu_configuration", label)
}
