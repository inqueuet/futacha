...
internal fun rememberCatalogHeadMetadataTitles(
    ...
    val hasHeader = remember(embeddedHtml) {
        embeddedHtml.any { it.placement == EmbeddedHtmlPlacement.Header }
    }
    ...
    LaunchedEffect(
        ...
        snapshotFlow {
            ...
            val visibleIds = buildList {
                when (displayStyle) {
                    CatalogDisplayStyle.Grid -> gridState.layoutInfo.visibleItemsInfo.forEach { visibleItem ->
                        items.getOrNull(visibleItem.index - headerOffset)?.id?.let(::add)
                    }
                    CatalogDisplayStyle.List -> listState.layoutInfo.visibleItemsInfo.forEach { visibleItem ->
                        items.getOrNull(visibleItem.index - headerOffset)?.id?.let(::add)
                    }
                }
            }
            visibleIds to isScrolling
        }
