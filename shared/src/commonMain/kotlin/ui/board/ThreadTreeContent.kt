...
internal suspend fun buildThreadTreeNodes(posts: List<Post>): List<ThreadTreeNode> {
    ...
    val orderIndex = HashMap<String, Int>(posts.size)
    val postIndex = HashMap<String, Post>(posts.size)
    posts.forEachIndexed { index, post ->
        orderIndex[post.id] = index
    ...
    posts.forEachIndexed { index, post ->
        ...
        val primaryParentId = post.quoteReferences
            ...
            .mapNotNull { targetId ->
                ...
                if (target.id == post.id || targetIndex >= index) {
                    null
                } else {
                    target.id
                }
    ...
    childrenByParentId.values.forEach { children ->
        children.sortBy { child -> orderIndex[child.id] ?: Int.MAX_VALUE }
    }

    suspend fun visitIteratively(root: Post) {
        val stack = ArrayDeque<Pair<Post, Int>>()
        ...
        while (stack.isNotEmpty()) {
            ...
            val (post, depth) = stack.removeLast()
            if (!visited.add(post.id)) continue
            result += ThreadTreeNode(post = post, depth = depth)
            val children = childrenByParentId[post.id].orEmpty()
            for (index in children.lastIndex downTo 0) {
                stack.addLast(children[index] to (depth + 1).coerceAtMost(12))
            }
        }
    }

    roots.forEach { visitIteratively(it) }
    posts.forEach { post ->
        if (post.id !in visited) {
            visitIteratively(post)
        }
    }
    return result
}
...
internal fun ThreadTreeContent(
    ...
    searchHighlightRanges: Map<Post, List<IntRange>>,
    ...
    val displayedPosts = remember(treeNodes) { treeNodes.map { it.post } }
