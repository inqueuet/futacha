...
internal fun CatalogScreenOverlayHost(
                    ...
                    CatalogMode.entries.forEach { mode ->
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                                .clickable {
                                    AnalyticsTracker.uiControl("catalog_mode", "カタログ表示モード: ${mode.label}")
    ...
    if (bindings.overlayState.isWatchWordsVisible) {
        WatchWordsSheet(
            globalWatchWords = bindings.globalWatchWords,
            boardWatchWordsOverride = bindings.boardWatchWordsOverride,
            effectiveBoardWatchWords = bindings.watchWords,
            boardName = bindings.board?.name.orEmpty(),
            isLoaded = bindings.isWatchWordsLoaded,
            onAddGlobalWord = bindings.overlayBindings.watchWordsCallbacks.onAddGlobalWord,
            onRemoveGlobalWord = bindings.overlayBindings.watchWordsCallbacks.onRemoveGlobalWord,
            onAddBoardWord = bindings.overlayBindings.watchWordsCallbacks.onAddBoardWord,
            onRemoveBoardWord = bindings.overlayBindings.watchWordsCallbacks.onRemoveBoardWord,
            onClearBoardOverride = bindings.overlayBindings.watchWordsCallbacks.onClearBoardOverride,
