                            ...
                            FilterChip(
                                ...
                                onClick = {
                                    AnalyticsTracker.uiControl(
                                        "storage_settings",
                                        if (selection == SaveDirectorySelection.MANUAL_INPUT) "スレ保存先を手入力に変更" else "スレ保存先をフォルダ選択に変更"
                                    )
                                    callbacks.onSaveDirectorySelectionChanged(selection)
                                },
                                label = {
                                    Text(
                                        when (selection) {
                    ...
                    when (state.effectiveSaveDirectorySelection) {
                        SaveDirectorySelection.MANUAL_INPUT -> {
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                TextButton(onClick = {
                                    AnalyticsTracker.uiControl("storage_settings", "手動保存先を初期値へ戻す")
                                    callbacks.onResetManualSaveDirectory()
                                }) {
                                    Text(text.resetManualSaveButtonLabel)
                                }
                                Button(onClick = {
                                    AnalyticsTracker.uiControl("storage_settings", "手入力の保存先を適用")
                        ...
                        SaveDirectorySelection.PICKER -> {
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                Button(
                                    onClick = {
                                        AnalyticsTracker.uiControl("storage_settings", "保存先フォルダ選択を開く")
                                        callbacks.onOpenSaveDirectoryPicker?.invoke()
                                    },
                                ...
                                if (state.saveDirectoryPickerState.showManualInputFallbackButton) {
                                    OutlinedButton(onClick = {
                                        AnalyticsTracker.uiControl("storage_settings", "保存先を手入力へ戻す")
    ...
    SettingsSection(
        ...
        ListItem(
            ...
            trailingContent = {
                Button(
                    onClick = {
                        AnalyticsTracker.uiControl("storage_settings", "画像キャッシュを削除")
                        cacheCallbacks.clearImageCache()
                    },
        ...
        ListItem(
            ...
            trailingContent = {
                OutlinedButton(
                    onClick = {
                        AnalyticsTracker.uiControl("storage_settings", "一時キャッシュを削除")
                        cacheCallbacks.clearTemporaryCache()
                    },
                    enabled = !cacheCallbacks.isCleanupInProgress()
                ) {
                    Text("掃除する")
...
internal fun GlobalSettingsStorageSection(
    ...
    SettingsSection(
        ...
        ListItem(
            ...
            trailingContent = {
                OutlinedButton(onClick = {
                    AnalyticsTracker.uiControl("storage_settings", "保存容量を再計算")
