...
internal fun resolveThreadAutoSaveAvailability(
    ...
    return when {
        ...
        minIntervalMillis > 0L &&
            elapsedThreadAutoSaveMillis(nowMillis, lastAutoSaveTimestampMillis) < minIntervalMillis ->
            ThreadAutoSaveAvailability.Throttled
        else -> ThreadAutoSaveAvailability.Ready
    }
}

private fun elapsedThreadAutoSaveMillis(nowMillis: Long, previousMillis: Long): Long {
    if (nowMillis < previousMillis) return Long.MIN_VALUE
    val elapsed = nowMillis - previousMillis
    return if (elapsed < 0L) Long.MAX_VALUE else elapsed
}

internal fun remainingThreadAutoSaveDelayMillis(
    nowMillis: Long,
    previousMillis: Long,
    minIntervalMillis: Long
): Long {
    val safeInterval = minIntervalMillis.coerceAtLeast(1_000L)
    val elapsed = elapsedThreadAutoSaveMillis(nowMillis, previousMillis)
    if (elapsed < 0L) return safeInterval
    val remaining = if (elapsed >= safeInterval) 0L else safeInterval - elapsed
    return remaining.coerceIn(1_000L, safeInterval)
}
