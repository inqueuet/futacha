...
internal fun buildAddBoardValidationState(
    ...
    val hasName = trimmedName.isNotEmpty() && trimmedName.length <= BOARD_MANAGEMENT_NAME_MAX_CHARS
    val hasUrl = trimmedUrl.isNotEmpty() && trimmedUrl.length <= BOARD_MANAGEMENT_URL_MAX_CHARS
    ...
    val isValidUrl = hasUrl && hasScheme && isSupportedBoardManagementUrl(trimmedUrl)
...
private fun isSupportedBoardManagementUrl(raw: String): Boolean {
    ...
    if (parsed.protocol.name.lowercase() !in setOf("http", "https")) return false
    val host = parsed.host.trimEnd('.')
    if (
        host.isBlank() || host.startsWith('.') || host.endsWith('.') ||
        ".." in host || host.any(Char::isWhitespace)
    ) return false
    if (host.matches(BOARD_MANAGEMENT_IPV4_REGEX)) {
        return host.split('.').all { it.toIntOrNull() in 0..255 }
    }
    return host.equals("localhost", ignoreCase = true) || '.' in host
}
...
internal fun toggleBoardSummaryPinned(
    boards: List<BoardSummary>,
    index: Int
): List<BoardSummary> {
    if (index !in boards.indices) return boards
    return boards.toMutableList().also { mutableBoards ->
        val board = mutableBoards[index]
        mutableBoards[index] = board.copy(pinned = !board.pinned)
    }
}
