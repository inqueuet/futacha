...
internal val FUTACHA_CHANGE_LOG_HTML: String = """
    ...
    <h2>10.7</h2>
    <ul>
        <li>としあき（仮）モードでカラーテーマをブラックにした際、更新履歴のバージョン見出しが見えなくなる問題を修正しました。</li>
        <li>ふたちゃモードととしあき（仮）モードのスレ本文の下に、削除されたレスの件数を表示するようにしました。スレ主・投稿者本人・管理者による削除と隔離を種類別に表示し、判別できない分は「種類不明」として表示します。</li>
