...
internal val FUTACHA_CHANGE_LOG_HTML: String = """
    ...
    <h2>11.6</h2>
    <ul>
        <li>ふたちゃモードで一度開いたスレッドを開き直したときに新しいレスが表示されない問題、カタログのサムネイルが表示されなくなることがある問題、送信時の確認が出ない問題、軽量モードが画像の読み込みに反映されない問題を修正しました。</li>
        <li>ふたちゃモードで、本文の上ではレスの長押し・タップが効かない問題、検索や画像一覧で移動した位置へ後から引き戻される問題、スレッド内検索中の「戻る」でスレッドが閉じる問題、保存済みスレッドの画面で「戻る」を押すとアプリが終了する問題を修正しました。</li>
        <li>ふたちゃモードで、別のスレッドへ移ったときに返信の添付画像が残る問題を修正しました。返信やスレ立ての送信中は画面を離れないようにし、更新でレス一覧が変わっても画像・動画の表示が閉じないようにしました。画像を大きく拡大したときは元の解像度で表示します。</li>
        <li>読み上げが画面の並びと異なる、NGや非表示にしたレスを読む、リストに触れると止まる問題を修正しました。</li>
        <li>カタログを開いたときに「表示できるスレッドがありません」やNG対象のスレッドが一瞬表示される問題と、更新後しばらく「更新中」のままになり次の更新ができない問題を修正しました。</li>
        <li>としあき（仮）モードで、スクロールした直後にタブの横スワイプが途中で止まる問題、画像NGのデータを保存できないとアプリが終了する問題、同じ画像が重複する画像ビューアーでアプリが終了することがある問題、返信を続けると設定が読み込まれなくなることがある問題を修正しました。</li>
        <li>としあき（仮）モードのスレッド・カタログ・画像一覧・画像ビューアーの表示と、削除レスの表示やキャッシュ検索の処理を軽くしました。何も操作していない間も画面の描画が続いていた問題を修正しました。</li>
        <li>iOSで、スレッドの表示やキャッシュの整理のあとにアプリが終了することがある問題、一部の文字を含むスレッドが空白で表示される問題、アイコンの切り替えでアプリが終了することがある問題、画像NGが働いていなかった問題を修正しました。</li>
        <li>iOSで履歴が多いと起動時に画面が白いまま待たされる問題を修正し、初期化に失敗したときは再試行できるようにしました。バックグラウンドの巡回通知が重複する問題と、条件によってバックグラウンドの巡回・過去ログ報告が動かない問題も修正しました。</li>
        <li>自動保存の保存データを合計2GBまでに抑え、古いものから整理するようにしました。モバイル回線では本文とサムネイルだけを保存します。保存したスレッドが多いと保存・一覧・削除ができなくなる問題を修正しました。</li>
        <li>回線が不安定なときに返信の送信が長く止まる問題、そうだねが二重に送られることがある問題、サムネイルの取得に失敗しただけで原寸画像を取りに行く問題を修正しました。変化のないスレッドの更新で通信量を減らしました。</li>
        <li>履歴のインポート・エクスポートで画面が止まる問題、2,000件を超える履歴をエクスポートできない問題、履歴の絞り込みの入力が重い問題を修正しました。</li>
        <li>バックグラウンド更新が回線の遅いときに長時間動き続ける問題、アプリの起動直後に更新の予定が作り直される問題、自動スクロールがバックグラウンドでも更新を続ける問題を修正しました。起動直後に配色が切り替わる、保存先の設定が戻ることがある問題も修正しました。</li>
        <li>iOSで回線が遅いとWebMなどの動画が再生できない問題を修正しました。動画編集で枠を動かすたびにコマを読み直す処理と、逆方向の追尾の処理を速くしました。</li>
        <li>Androidで長時間の一括保存中にアプリが終了することがある問題と、フォルダを選ぶ途中で画面を閉じると権限が解放されない問題を修正しました。</li>
        <li>Wear OSで、タイルや読み上げの通知が古い表示のまま残る問題、スマホと未接続の間に押した操作が後から実行される問題、「戻る」でアプリが終了する問題を修正しました。</li>
        <li>デスクトップ版で、保存中に終了すると履歴などが壊れることがある問題、ファイルの処理中に画面が止まる問題、別のアプリを選んだだけで読み上げが止まりアプリロックがかかる問題、有線LANで「Wi-Fiのみ」の更新が動かない問題を修正しました。</li>
    </ul>
    <h2>11.5</h2>
    <ul>
