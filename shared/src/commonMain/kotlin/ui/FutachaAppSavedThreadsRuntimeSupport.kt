...
internal suspend fun shouldResetInaccessibleManualSaveBookmark(
    ...
    return !runSuspendCatchingPreservingCancellation {
        fileSystem.exists(manualSaveLocation, "")
    }.getOrDefault(false)
}
...
internal suspend fun resolveActiveSavedThreadsRepository(
    ...
    val currentCount = runSuspendCatchingPreservingCancellation {
        inputs.currentRepository.getThreadCount()
    }.getOrDefault(0)
    val legacyCount = inputs.legacyRepository?.let {
        runSuspendCatchingPreservingCancellation { it.getThreadCount() }.getOrDefault(0)
    } ?: 0
