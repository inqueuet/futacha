...
internal const val OPERATION_BACKUP_CLEANUP_MAX_SCANNED_FILES = 20_000
...
internal suspend fun SavedThreadRepository.cleanupStaleOperationBackups() {
    ...
    val shouldRun = backupCleanupMutex.withLock {
        if (
            ...
            !hasEpochIntervalElapsed(
                nowMillis,
                lastOperationBackupCleanupEpochMillis,
                OPERATION_BACKUP_CLEANUP_MIN_INTERVAL_MILLIS
            )
        ) {
            false
        } else {
    ...
    if (!shouldRun) return
    val cutoffMillis = saturatingEpochSubtract(nowMillis, OPERATION_BACKUP_RETENTION_MILLIS)
    val fileNames = try {
    ...
    fileNames.asSequence().take(OPERATION_BACKUP_CLEANUP_MAX_SCANNED_FILES).forEach { fileName ->
