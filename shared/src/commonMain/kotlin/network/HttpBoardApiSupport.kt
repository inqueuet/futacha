...
private const val POSTING_FAILURE_SCAN_MAX_CHARS = 64 * 1024
private const val POSTING_FAILURE_NORMALIZATION_MAX_CHARS = 8 * 1024
private const val POSTING_FAILURE_DETAIL_MAX_CHARS = 300
private const val POSTING_FAILURE_LINE_MAX_CHARS = 1024
private const val POSTING_FAILURE_SCAN_MAX_LINES = 256
...
internal fun isSuccessfulHttpBoardApiSaidaneResponse(body: String): Boolean {
    ...
    while (end >= start && body[end].isWhitespace()) end -= 1
    if (start > end) return false
    for (index in start..end) {
        if (!body[index].isDigit()) return false
    }
    return true
}
    ...
    if (looksLikeHttpBoardApiJson(normalized)) {
        ...
        return buildString {
            ...
            if (!message.isNullOrBlank()) {
                append(", message=")
                append(message.take(POSTING_FAILURE_DETAIL_MAX_CHARS))
            }
        }
    }
...
private fun httpBoardApiHumanReadableLines(body: String): Sequence<String> {
    ...
    return text
        ...
        .replace('\u3000', ' ')
        .lineSequence()
        .take(POSTING_FAILURE_SCAN_MAX_LINES)
        .map {
            it.take(POSTING_FAILURE_LINE_MAX_CHARS)
                .trim()
                .replace(HUMAN_READABLE_WHITESPACE_REGEX, " ")
        }
        .filter { it.isNotEmpty() }
}

private fun extractHttpBoardApiHumanReadableErrorLine(body: String): String? {
    val lines = httpBoardApiHumanReadableLines(body)
        .take(POSTING_FAILURE_SCAN_MAX_LINES)
        .toList()
    ...
    if (tailLines.isEmpty()) {
        return firstLine.take(POSTING_FAILURE_DETAIL_MAX_CHARS)
    }
...
internal fun extractHttpBoardApiPostingWaitSeconds(detail: String): Long? {
    val normalized = detail
        .take(POSTING_FAILURE_NORMALIZATION_MAX_CHARS)
        .trim()
        .replace('\u3000', ' ')
        .lowercase()
...
internal fun buildHttpBoardApiPostingFailureMessage(prefix: String, detail: String): String {
    val trimmed = detail.take(POSTING_FAILURE_DETAIL_MAX_CHARS).trim()
    if (trimmed.isEmpty()) {
        return prefix
    }
