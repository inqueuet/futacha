...
class PersistentCookieStorage(
            ...
            mutex.withLock {
                ...
                if (domain == null) {
                    Logger.w(
                        ...
                        "Rejected cookie with invalid domain for host=${requestUrl.host.take(255)}"
                    )
                    return@withLock
                }
                if (!isPersistentCookieNameAllowed(cookie.name) || !isPersistentCookieValueAllowed(cookie.value)) {
                    Logger.w(
                        "PersistentCookieStorage",
                        "Rejected cookie due to invalid size/format for host=${requestUrl.host.take(255)}"
                    )
                    return@withLock
                }
                val path = normalizePersistentCookiePath(cookie.path)
                if (!isPersistentCookiePathAllowed(path)) {
                    Logger.w(
                        "PersistentCookieStorage",
                        "Rejected cookie due to invalid path for host=${requestUrl.host.take(255)}"
                    )
                    return@withLock
                }
                ...
                val stored = StoredCookie(
                    ...
                    domain = domain,
                    path = path,
                    hostOnly = cookie.domain.isNullOrBlank(),
    ...
    override suspend fun get(requestUrl: Url): List<Cookie> {
        ...
        val result = mutex.withLock {
            ...
            sourceCookies.values.filter { stored ->
                ...
                persistentCookieDomainMatches(host, stored.domain, stored.hostOnly) &&
                ...
                Cookie(
                    ...
                    maxAge = stored.expiresAtMillis?.let { persistentCookieMaxAgeSeconds(it, now) },
    ...
    suspend fun <T> commitEvenOnFailure(block: suspend () -> T): T {
        ...
        try {
            ...
            persistTransactionBestEffort(transactionId, "completed request")
            return result
        } catch (t: Throwable) {
    ...
    private suspend fun persistTransactionBestEffort(transactionId: Long?, context: String) {
        try {
            persistTransaction(transactionId)
        } catch (cancelled: CancellationException) {
            throw cancelled
        } catch (error: Exception) {
            // The remote operation has already completed. Converting a cookie
            // disk failure into an operation failure can cause callers to retry
            // non-idempotent votes, deletions, or posts. Keep the committed
            // in-memory cookie state and let a later mutation retry persistence.
            Logger.e(
                "PersistentCookieStorage",
    ...
    private fun purgeExpiredLocked(
        ...
        if (!force && isGlobalCookieMap &&
            !hasEpochIntervalElapsed(now, lastPurgeTimeMillis, purgeIntervalMillis)
        ) {
            return false
        }
...
internal fun persistentCookieMaxAgeSeconds(expiresAtMillis: Long, nowMillis: Long): Int {
    if (expiresAtMillis <= nowMillis) return 0
    val maxRepresentableMillis = Int.MAX_VALUE.toLong() * 1_000L
    val remainingMillis = if (nowMillis >= 0L) {
        (expiresAtMillis - nowMillis).coerceAtMost(maxRepresentableMillis)
    } else {
        val threshold = Long.MAX_VALUE + nowMillis
        if (expiresAtMillis > threshold) maxRepresentableMillis
        else (expiresAtMillis - nowMillis).coerceAtMost(maxRepresentableMillis)
    }
