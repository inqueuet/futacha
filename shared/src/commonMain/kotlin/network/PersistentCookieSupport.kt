...
private val ALLOWED_SHARED_COOKIE_PARENT_DOMAINS = setOf(
    "2chan.net",
    "google.com",
    "iqdb.org",
    "saucenao.com",
...
internal fun resolvePersistentCookieExpiresAt(cookie: Cookie, now: Long): Long? {
    ...
    if (maxAgeSeconds != null) {
        if (maxAgeSeconds > 0) {
            return saturatingEpochAdd(now, maxAgeSeconds.seconds.inWholeMilliseconds)
        }
        if (maxAgeSeconds < 0) {
            return null
        }
...
internal fun resolvePersistentCookieDomain(setCookieDomain: String?, requestHost: String): String? {
    ...
    val isParentDomain =
        candidate in ALLOWED_SHARED_COOKIE_PARENT_DOMAINS && request.endsWith(".$candidate") &&
            !isLikelyPersistentCookieIpAddress(candidate)
...
internal fun isPersistentCookiePathAllowed(path: String): Boolean {
    return path.length <= MAX_COOKIE_PATH_LENGTH && path.startsWith('/') &&
        path.none { it == '\u0000' || it == '\r' || it == '\n' }
}

internal fun persistentCookieDomainMatches(
    host: String,
    cookieDomain: String,
...
internal fun isStoredPersistentCookieAllowed(stored: StoredCookie): Boolean {
    ...
    if (!isPersistentCookiePathAllowed(normalizePersistentCookiePath(stored.path))) return false
    return true
}
