...
private const val AI_NOTIFICATION_MAX_REQUEST_CHARS = 256_000
private const val AI_NOTIFICATION_MAX_RESPONSE_CHARS = 256_000
private const val AI_NOTIFICATION_MAX_ERROR_CHARS = 8_192
...
private suspend fun requestFoundationModelsResponse(
    ...
    if (sourceText.length > AI_NOTIFICATION_MAX_REQUEST_CHARS) {
        return Result.failure(IllegalArgumentException("Apple Intelligence request is too large"))
    }
    ...
    val observer = NSNotificationCenter.defaultCenter.addObserverForName(
        ...
        val rawError = (userInfo[errorKey] as? String).orEmpty()
        val rawText = (userInfo[textKey] as? String).orEmpty()
        if (
            rawError.length > AI_NOTIFICATION_MAX_ERROR_CHARS ||
            rawText.length > AI_NOTIFICATION_MAX_RESPONSE_CHARS
        ) {
            deferred.complete(Result.failure(IllegalStateException("Apple Intelligence response is too large")))
