...
private const val CATALOG_FETCH_ROWS_KEY = "catalog_fetch_rows"
...
private const val UPDATE_CHECK_KEY = "update_check_enabled"
private const val WATCH_ALERT_KEY = "watch_alert_enabled"
...
private const val TELEMETRY_COLLECTION_KEY = "telemetry_collection_enabled"
...
private const val THREAD_GALLERY_THUMBNAIL_MODE_KEY = "thread_gallery_thumbnail_mode"
...
private const val COMPACT_THREAD_HEADER_ENABLED_KEY = "compact_thread_header_enabled"
...
private const val BOARD_WATCH_WORDS_KEY = "board_watch_words_json"
...
private class IosPlatformStateStorage : PlatformStateStorage {
    ...
    private val catalogFetchRowsState = MutableStateFlow<String?>(null)
    private val privacyFilterState = MutableStateFlow(false)
    private val backgroundRefreshState = MutableStateFlow(false)
    private val updateCheckState = MutableStateFlow(true)
    private val watchAlertState = MutableStateFlow(false)
    ...
    // The public flow waits for deferred loading, but keep the backing state
    // fail-closed too so no internal observer can expose an optimistic ON value.
    private val telemetryCollectionState = MutableStateFlow(false)
    ...
    private val threadGalleryThumbnailModeState = MutableStateFlow<String?>(null)
    ...
    private val compactThreadHeaderEnabledState = MutableStateFlow(false)
    ...
    private val boardWatchWordsState = MutableStateFlow<String?>(null)
    ...
    override val updateCheckEnabled: Flow<Boolean> = awaitDeferredInitialLoad(updateCheckState)
    override val watchAlertEnabled: Flow<Boolean> = awaitDeferredInitialLoad(watchAlertState)
    ...
    override val telemetryCollectionEnabled: Flow<Boolean> = awaitDeferredInitialLoad(telemetryCollectionState)
    ...
    override val threadGalleryThumbnailMode: Flow<String?> = awaitDeferredInitialLoad(threadGalleryThumbnailModeState)
    ...
    override val compactThreadHeaderEnabled: Flow<Boolean> =
        awaitDeferredInitialLoad(compactThreadHeaderEnabledState)
    ...
    override val catalogFetchRows: Flow<String?> = awaitDeferredInitialLoad(catalogFetchRowsState)
    ...
    override val boardWatchWordsJson: Flow<String?> = awaitDeferredInitialLoad(boardWatchWordsState)
    ...
    private suspend fun loadDeferredInitialState() {
        val stringValues = mapOf(
            ...
            CATALOG_FETCH_ROWS_KEY to readStringState(CATALOG_FETCH_ROWS_KEY),
            ...
            THREAD_GALLERY_THUMBNAIL_MODE_KEY to readStringState(THREAD_GALLERY_THUMBNAIL_MODE_KEY),
            ...
            BOARD_WATCH_WORDS_KEY to readStringState(BOARD_WATCH_WORDS_KEY),
        ...
        val booleanValues = mapOf(
            ...
            UPDATE_CHECK_KEY to readBooleanState(UPDATE_CHECK_KEY, defaultValue = true),
            WATCH_ALERT_KEY to readBooleanState(WATCH_ALERT_KEY),
            ...
            TELEMETRY_COLLECTION_KEY to readBooleanState(TELEMETRY_COLLECTION_KEY),
            COMPACT_THREAD_HEADER_ENABLED_KEY to readBooleanState(COMPACT_THREAD_HEADER_ENABLED_KEY)
        )
        ...
        applyDeferredStringState(CATALOG_FETCH_ROWS_KEY, stringValues, catalogFetchRowsState)
        ...
        applyDeferredBooleanState(UPDATE_CHECK_KEY, booleanValues, updateCheckState)
        applyDeferredBooleanState(WATCH_ALERT_KEY, booleanValues, watchAlertState)
        ...
        applyDeferredBooleanState(TELEMETRY_COLLECTION_KEY, booleanValues, telemetryCollectionState)
        ...
        applyDeferredStringState(
            THREAD_GALLERY_THUMBNAIL_MODE_KEY,
            stringValues,
            threadGalleryThumbnailModeState
        )
        ...
        applyDeferredBooleanState(
            ...
            booleanValues,
            compactThreadHeaderEnabledState
        )
        ...
        applyDeferredStringState(BOARD_WATCH_WORDS_KEY, stringValues, boardWatchWordsState)
    ...
    private fun seedFrom(seedBundles: AppStateSeedBundles) {
        ...
        seedRequiredBooleanState(TELEMETRY_COLLECTION_KEY, true, telemetryCollectionState)
    ...
    override suspend fun updateUpdateCheckEnabled(enabled: Boolean) {
        updateBooleanState(UPDATE_CHECK_KEY, enabled, updateCheckState)
    }

    override suspend fun updateWatchAlertEnabled(enabled: Boolean) {
        updateBooleanState(WATCH_ALERT_KEY, enabled, watchAlertState)
    }
    ...
    override suspend fun updateTelemetryCollectionEnabled(enabled: Boolean) {
        updateBooleanState(TELEMETRY_COLLECTION_KEY, enabled, telemetryCollectionState)
    }
    ...
    override suspend fun updateCompactThreadHeaderEnabled(enabled: Boolean) {
        updateBooleanState(COMPACT_THREAD_HEADER_ENABLED_KEY, enabled, compactThreadHeaderEnabledState)
    }
    ...
    override suspend fun updateCatalogFetchRows(rows: String) {
        updateStringState(CATALOG_FETCH_ROWS_KEY, rows, catalogFetchRowsState)
    }
    ...
    override suspend fun updateBoardWatchWordsJson(value: String) {
        updateLargeStringState(BOARD_WATCH_WORDS_KEY, value, boardWatchWordsState)
    }
