...
private fun NSData.toByteArray(): ByteArray {
    require(length <= WEBKIT_THUMBNAIL_DISK_CACHE_MAX_SINGLE_FILE_BYTES.toULong()) {
        "Video thumbnail exceeds its permitted in-memory size"
    }
...
private suspend fun rememberWebKitThumbnail(cacheKey: String, data: NSData) {
    if (data.length > WEBKIT_THUMBNAIL_DISK_CACHE_MAX_SINGLE_FILE_BYTES.toULong()) return
    webKitThumbnailCacheMutex.withLock {
        webKitThumbnailCache.remove(cacheKey)?.let { previous ->
            webKitThumbnailCacheBytes = (webKitThumbnailCacheBytes - previous.length.toLong()).coerceAtLeast(0L)
        }
        webKitThumbnailCache[cacheKey] = data
        webKitThumbnailCacheBytes += data.length.toLong()
        while (
            webKitThumbnailCache.size > WEBKIT_THUMBNAIL_CACHE_MAX_ENTRIES ||
            webKitThumbnailCacheBytes > WEBKIT_THUMBNAIL_MEMORY_CACHE_MAX_BYTES
        ) {
            val eldestKey = webKitThumbnailCache.keys.firstOrNull() ?: break
            webKitThumbnailCache.remove(eldestKey)?.let { removed ->
                webKitThumbnailCacheBytes =
                    (webKitThumbnailCacheBytes - removed.length.toLong()).coerceAtLeast(0L)
            }
        }
    }
}
...
private suspend fun readDiskCachedWebKitThumbnail(cacheKey: String): NSData? {
    return withContext(AppDispatchers.io) {
        ...
        val size = (fileManager.attributesOfItemAtPath(path, error = null)?.get(NSFileSize) as? NSNumber)
            ?.longValue
            ?: return@withContext null
        if (size !in 1L..WEBKIT_THUMBNAIL_DISK_CACHE_MAX_SINGLE_FILE_BYTES) {
            // The cache is disposable. Remove malformed/oversized entries
            // before Foundation attempts to allocate their complete payload.
            fileManager.removeItemAtPath(path, error = null)
            return@withContext null
        }
        readBoundedWebKitThumbnailData(path, size.toInt())
    }
}

private fun readBoundedWebKitThumbnailData(path: String, expectedSize: Int): NSData? {
    ...
    try {
        memScoped {
            ...
            while (offset < expectedSize) {
                error.value = null
                val requested = minOf(64 * 1024, expectedSize - offset)
            ...
            // Detect growth after stat() without asking Foundation to allocate
            // the expanded file in one operation.
    ...
    return bytes.usePinned { pinned ->
        NSData.create(bytes = pinned.addressOf(0), length = offset.toULong())
    }
}
    ...
    withContext(AppDispatchers.io) {
        ...
        memScoped {
            ...
            if (data.writeToFile(path, options = NSDataWritingAtomic, error = error.ptr)) {
                pruneWebKitThumbnailDiskCache(directory)
            }
        }
    }
}

private data class WebKitThumbnailDiskEntry(
    val path: String,
    val sizeBytes: Long,
    val modifiedAtSeconds: Double
)

private fun pruneWebKitThumbnailDiskCache(directory: String) {
    val fileManager = NSFileManager.defaultManager
    val entries = fileManager.contentsOfDirectoryAtPath(directory, error = null)
        ?.filterIsInstance<String>()
        ?.take(WEBKIT_THUMBNAIL_DISK_CACHE_MAX_INSPECTION_ENTRIES)
        ?.mapNotNull { name ->
            ...
            if (fileType == NSFileTypeDirectory) return@mapNotNull null
            WebKitThumbnailDiskEntry(
                path = path,
                sizeBytes = ((attributes[NSFileSize] as? NSNumber)?.longValue ?: 0L).coerceAtLeast(0L),
                modifiedAtSeconds = (attributes[NSFileModificationDate] as? NSDate)?.timeIntervalSince1970 ?: 0.0
            )
        }
        ?.sortedByDescending(WebKitThumbnailDiskEntry::modifiedAtSeconds)
        ?: return
    var retainedBytes = 0L
    entries.forEachIndexed { index, entry ->
        val exceedsCount = index >= WEBKIT_THUMBNAIL_DISK_CACHE_MAX_ENTRIES
        val exceedsBytes = entry.sizeBytes > WEBKIT_THUMBNAIL_DISK_CACHE_MAX_BYTES - retainedBytes
...
private const val WEBKIT_THUMBNAIL_MEMORY_CACHE_MAX_BYTES = 32L * 1024L * 1024L
private const val WEBKIT_THUMBNAIL_DISK_CACHE_DIR = "webm_thumbnails"
private const val WEBKIT_THUMBNAIL_DISK_CACHE_MAX_ENTRIES = 128
private const val WEBKIT_THUMBNAIL_DISK_CACHE_MAX_BYTES = 64L * 1024L * 1024L
private const val WEBKIT_THUMBNAIL_DISK_CACHE_MAX_SINGLE_FILE_BYTES = 8L * 1024L * 1024L
private const val WEBKIT_THUMBNAIL_DISK_CACHE_MAX_INSPECTION_ENTRIES = 4_096
