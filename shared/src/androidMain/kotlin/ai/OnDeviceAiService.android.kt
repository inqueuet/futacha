    ...
    private fun launchRequest(requestId: Int, block: suspend () -> Unit) {
        ...
        val job = serviceScope.launch {
            try {
                ...
                // A duplicate id cancels and replaces the previous job. The
                // previous job must not remove the replacement from the map
                // when its cancellation finally runs.
...
private class AndroidAiRemoteServiceSession {
    ...
    // Context.bindService can synchronously cross into ActivityManager and wait
    // for service/package state.  Keep that call away from the app's UI Looper;
    // ServiceConnection callbacks are marshalled back to the main handler below.
    private val bindScope = CoroutineScope(SupervisorJob() + AppDispatchers.io)
...
private fun Bundle.putPosts(
    ...
    maxTotalMessageChars: Int = (posts.size.toLong() * maxMessageChars.toLong())
