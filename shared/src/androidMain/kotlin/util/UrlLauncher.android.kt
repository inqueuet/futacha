            ...
            try {
                ...
                if (request == null) {
                    ...
                    if (parsed == null || parsed.scheme.isNullOrBlank()) {
                        Logger.w("UrlLauncher", "Invalid URL (${describeUrlForLog(request.normalizedUrl)})")
                        Toast.makeText(context, "リンクを開けません", Toast.LENGTH_SHORT).show()
                    } else {
                        ...
                        try {
                            ...
                            Logger.w("UrlLauncher", "No activity can handle URL (${describeUrlForLog(request.normalizedUrl)})")
                ...
                Logger.e(
                    "UrlLauncher",
                    "Failed to open URL (${describeUrlForLog(url)}), type=${describeFailureForLog(e)}"
                )
                Toast.makeText(context, "リンクを開けません", Toast.LENGTH_SHORT).show()
            }
        }
