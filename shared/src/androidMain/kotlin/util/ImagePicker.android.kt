...
suspend fun readImageDataFromUri(
    context: Context,
    uri: Uri,
    maxBytes: Long = MAX_PICKED_IMAGE_BYTES
): ImageData? {
    if (maxBytes <= 0L || maxBytes > Int.MAX_VALUE.toLong()) {
        Logger.w("ImagePicker", "Invalid picker byte limit: $maxBytes")
        return null
    }
    return withTimeoutOrNull(IMAGE_PICKER_READ_TIMEOUT_MILLIS) {
        readImageDataFromUriWithinTimeout(context, uri, maxBytes)
    } ?: run {
...
private suspend fun readImageDataFromUriWithinTimeout(
    context: Context,
    uri: Uri,
    maxFileSize: Long
): ImageData? {
