...
private class DesktopPlatformStateStorage(private val dataStore: DataStore<Preferences>) : PlatformStateStorage {
    ...
    private suspend fun updateStringPreference(
        ...
        try {
            ...
            Logger.e("DesktopPlatformStateStorage", "Failed to update $logLabel: ${e.message}")
    ...
    override suspend fun seedIfEmpty(seedBundles: AppStateSeedBundles) {
        try {
            dataStore.edit { prefs -> prefs.seedFrom(seedBundles) }
        } catch (e: Exception) {
            rethrowIfCancellation(e)
            Logger.e("DesktopPlatformStateStorage", "Failed to seed data: ${e.message}")
