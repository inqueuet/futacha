...
class AppStateStoreTest {
    ...
    @Test
    fun updateCheckDefaultsOnAndCanBeDisabled() = runBlocking {
        val store = AppStateStore(FakePlatformStateStorage())

        assertTrue(store.isUpdateCheckEnabled.first())
        store.setUpdateCheckEnabled(false)

        assertEquals(false, store.isUpdateCheckEnabled.first())
    }
    ...
    @Test
    fun seedIfEmpty_repairsMalformedBoardsButPreservesValidEmptyList() = runBlocking {
        val defaultBoard = BoardSummary(
            id = "b",
            name = "Board",
            category = "cat",
            url = "https://may.2chan.net/b/",
        ...
        val corruptedStorage = FakePlatformStateStorage().apply {
            boardsState.value = "{not-valid-json"
        }
        val corruptedStore = AppStateStore(corruptedStorage)

        corruptedStore.seedIfEmpty(
            AppStateSeedDefaults(boards = listOf(defaultBoard), history = emptyList())
        )

        assertEquals(listOf(defaultBoard), corruptedStore.boards.first())

        val intentionallyEmptyStorage = FakePlatformStateStorage().apply {
            boardsState.value = "[]"
        }
        val intentionallyEmptyStore = AppStateStore(intentionallyEmptyStorage)
        intentionallyEmptyStore.seedIfEmpty(
            AppStateSeedDefaults(boards = listOf(defaultBoard), history = emptyList())
        )
        assertTrue(intentionallyEmptyStore.boards.first().isEmpty())
    }
    ...
    @Test
    fun historyFileStore_doesNotCacheProvisionalEmptyBeforeSeedArrives() = runBlocking {
        val storage = FakePlatformStateStorage()
        val fileSystem = InMemoryFileSystem()
        val store = AppStateStore(
            storage = storage,
        ...
        store.seedIfEmpty(
            AppStateSeedDefaults(
                boards = emptyList(),
                history = listOf(historyEntry(threadId = "seeded"))
            )
        )

        assertEquals(listOf("seeded"), store.history.first { it.isNotEmpty() }.map { it.threadId })
        assertTrue(fileSystem.exists("private/history_store/manifest.json"))
    }

    @Test
    fun historyFileStore_reusesSnapshotAcrossIndependentCollectors() = runBlocking {
        val storage = FakePlatformStateStorage()
        ...
        val readsAfterFirstCollector = fileSystem.readCount
        assertTrue(readsAfterFirstCollector > 0)

        assertEquals(listOf("111"), reader.history.first().map { it.threadId })
        assertEquals(readsAfterFirstCollector, fileSystem.readCount)
    }
    ...
    fun preferenceSetters_preservePreviousValuesWhenPersistenceFails() = runBlocking {
        ...
        store.setTelemetryCollectionEnabled(true)
        ...
        storage.failTelemetryCollectionUpdate = true
        ...
        store.setTelemetryCollectionEnabled(false)
        ...
        assertEquals(true, store.isTelemetryCollectionEnabled.first())
    }

    @Test
    fun telemetryPreference_canBeDisabledAndIsNotResetBySeeding() = runBlocking {
        val storage = FakePlatformStateStorage()
        val store = AppStateStore(storage)
        ...
        assertEquals(false, store.isTelemetryCollectionEnabled.first())
    }
    ...
    fun setters_normalizeDeleteKeyDirectoryGridColumnsAndEnums() = runBlocking {
        ...
        store.setThreadGalleryThumbnailMode(ThreadGalleryThumbnailMode.Fit)
        ...
        assertEquals(200, store.catalogFetchRows.first())
        ...
        assertEquals(ThreadGalleryThumbnailMode.Fit, store.threadGalleryThumbnailMode.first())
    }
    ...
    fun historyMutations_preserveOrderAndApplyEntryUpdates() = runBlocking {
        ...
        // A later metadata/visit write is built from a stale history snapshot
        // in the real thread loader. It must not replace the independent
        // scroll position update.
        store.upsertHistoryEntry(
            updatedEntry.copy(
                title = "metadata-refresh",
                lastReadItemIndex = 0,
                lastReadItemOffset = 0,
                lastVisitedEpochMillis = updatedEntry.lastVisitedEpochMillis + 1_000L
            )
        )
        val afterMetadataRefresh = store.history.first().first { it.threadId == "222" }
        assertEquals("metadata-refresh", afterMetadataRefresh.title)
        assertEquals(7, afterMetadataRefresh.lastReadItemIndex)
        assertEquals(14, afterMetadataRefresh.lastReadItemOffset)

        store.prependOrReplaceHistoryEntries(
            listOf(
                afterMetadataRefresh.copy(
                    title = "batch-refresh",
                    lastReadItemIndex = 0,
                    lastReadItemOffset = 0
                )
        ...
        val afterBatchRefresh = store.history.first().first { it.threadId == "222" }
        assertEquals("batch-refresh", afterBatchRefresh.title)
        assertEquals(7, afterBatchRefresh.lastReadItemIndex)
        assertEquals(14, afterBatchRefresh.lastReadItemOffset)
    ...
    @Test
    fun delayedMetadataAndScrollUpdatesDoNotRecreateDeletedHistory() = runBlocking {
        val store = AppStateStore(FakePlatformStateStorage())
        val entry = historyEntry("111")
        store.setHistory(listOf(entry))
        store.removeHistoryEntry(entry)

        store.upsertHistoryEntry(entry.copy(title = "stale metadata"))
        store.updateHistory { listOf(entry.copy(title = "stale bridge")) }
        store.updateHistoryScrollPositionImmediately(
            AppStateHistoryScrollUpdateRequest(
        ...
        store.setHistory(listOf(entry))
        store.clearHistory()
        store.updateHistory { listOf(entry.copy(title = "stale clear bridge")) }
        assertEquals(emptyList(), store.history.first())

        // Explicit restore/import may preserve its archived visit timestamp;
        // later bridge reconciliation must not filter an already-restored row.
        store.setHistory(listOf(entry.copy(title = "explicit restore")))
        store.updateHistory { current ->
            current.map { restored -> restored.copy(title = "restored metadata") }
        }
        assertEquals(listOf("111"), store.history.first().map { it.threadId })
        assertEquals("restored metadata", store.history.first().single().title)
    }

    @Test
    fun clearingSplitHistoryAlsoPersistsExplicitEmptyLegacyValue() = runBlocking {
        val storage = FakePlatformStateStorage()
        val fileSystem = InMemoryFileSystem()
        val store = AppStateStore(
            storage = storage,
        ...
        store.setHistory(listOf(historyEntry("111")))
        storage.historyState.value = encodeAppStateHistory(listOf(historyEntry("legacy")), json)
        fileSystem.writeString("private/history_store/entries/orphan.json", "stale").getOrThrow()
        fileSystem.writeString("private/history_store/entries/orphan-dir/payload", "stale").getOrThrow()

        store.setHistory(emptyList())

        assertEquals("[]", storage.historyState.value)
        assertEquals(emptyList(), store.history.first())
        assertTrue(!fileSystem.exists("private/history_store/entries/orphan.json"))
        assertTrue(!fileSystem.exists("private/history_store/entries/orphan-dir"))
    }
    ...
    fun preferenceFacade_forwardsPreferenceAndSelfPostOperations() = runBlocking {
        ...
        val facade = buildAppStatePreferenceFacade(
            ...
            setCatalogFetchRowsImpl = { calls += "fetchRows:$it" },
        ...
        facade.setCatalogFetchRows(100)
...
internal class FakePlatformStateStorage : BaseInMemoryPlatformStateStorage() {
    ...
    var failTelemetryCollectionUpdate = false
    ...
    override suspend fun updateTelemetryCollectionEnabled(enabled: Boolean) {
        if (failTelemetryCollectionUpdate) error("telemetry collection write failed")
        super.updateTelemetryCollectionEnabled(enabled)
    }
