...
class SavedThreadExtensionsTest {
    @Test
    fun toThreadPage_resolvesSharedBasePathOnlyOnce() = runBlocking {
        val metadata = savedMetadata(
            storageId = "b_123",
            imagePath = "b/src/123.jpg",
            thumbnailPath = "b/thumb/123s.jpg"
        )
        val fileSystem = CountingResolveFileSystem()

        metadata.toThreadPage(fileSystem = fileSystem)

        assertEquals(1, fileSystem.resolveCount)
    }
...
private class CountingResolveFileSystem(
    ...
    override fun resolveAbsolutePath(relativePath: String): String {
        resolveCount += 1
        return delegate.resolveAbsolutePath(relativePath)
    }
}
