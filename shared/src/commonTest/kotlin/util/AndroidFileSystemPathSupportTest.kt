...
class AndroidFileSystemPathSupportTest {
    ...
    @Test
    fun resolveAndroidAbsolutePath_onlyEvaluatesTheRequiredStorageRoot() {
        val calls = mutableListOf<String>()
        fun resolve(path: String): String = resolveAndroidAbsolutePath(
            relativePath = path,
            appDataDirectory = { calls += "app"; appDataDirectory },
            privateAppDataDirectory = { calls += "private"; privateAppDataDirectory },
            publicDocumentsDirectory = { calls += "documents"; publicDocumentsDirectory },
            publicDownloadsDirectory = { calls += "downloads"; publicDownloadsDirectory }
        )

        assertEquals("/already/absolute.json", resolve("/already/absolute.json"))
        assertEquals(emptyList(), calls)

        assertEquals(
            "$privateAppDataDirectory/history_store/manifest.json",
            resolve("private/history_store/manifest.json")
        )
        assertEquals(listOf("private"), calls)

        calls.clear()
        assertEquals("$publicDownloadsDirectory/saved_threads", resolve("Downloads"))
        assertEquals(listOf("downloads"), calls)
    }
}
